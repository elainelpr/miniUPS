...
void *handler(void *arg){
  ...
  int world_fd = invoke_server(port);
...
int length = lines.length();
  ...
  send(world_fd, &length, sizeof(length), 0);
  //cout << "Content of lines is :" << lines << endl;
  send(world_fd, lines.c_str(),lines.length(), 0);
  char buffer[10240];
  int lengthRSP;
  recv(world_fd, &lengthRSP, sizeof(lengthRSP), 0);
  recv(world_fd, buffer, lengthRSP, 0);
